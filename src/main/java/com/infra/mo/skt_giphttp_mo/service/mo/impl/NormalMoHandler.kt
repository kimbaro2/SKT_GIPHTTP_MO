...
class NormalMoHandler(
    ...
    private suspend fun doProcessSMReqSimpleInHandler(qItem: QITEM, context: MoServiceContext) {
        ...
        if (nRetMO) {
            ...
            witcomLog.c_write(loggerName, Level.INFO, String.format("[InsqStat 추적] 호출클래스=%s, 호출라인=%d, nErrorID=%d, nStatusNo=%d", "NormalMoHandler", 125, ERRORID_CP_MO_LIMIT, ST_GIP_MO_LIMIT), workerThreadId)
            smsQLib.InsqStat(
        ...
        if (nRetGIVE) {
            ...
            witcomLog.c_write(loggerName, Level.INFO, String.format("[InsqStat 추적] 호출클래스=%s, 호출라인=%d, nErrorID=%d, nStatusNo=%d", "NormalMoHandler", 152, ERRORID_CP_GIVEBILL_LIMIT, ST_GIP_MT_LIMIT_GIFT), workerThreadId)
            smsQLib.InsqStat(
                gstQItem, MESSAGE_MO, 0, gServerID, MODULEID_GIPEVENT_C, SERVICEID_GIPEVENT,
        ...
        if (gMOTRBILL) {
            if (isRelayMo) {
                ...
                if (relayRes < 0) {
                    dbInsertOk = false
                    witcomLog.c_write(loggerName, Level.INFO, String.format("[InsqStat 추적] 호출클래스=%s, 호출라인=%d, nErrorID=%d, nStatusNo=%d", "NormalMoHandler", 198, ERRORID_CP_MO_FAIL, ST_DB_INS_FAIL_GIPMOCALLINFO), workerThreadId)
                    smsQLib.InsqStat(
                        gstQItem, MESSAGE_MO, 0, gServerID, MODULEID_GIPEVENT_C, SERVICEID_GIPEVENT,
                ...
                if (callRes < 0) {
                    dbInsertOk = false
                    witcomLog.c_write(loggerName, Level.INFO, String.format("[InsqStat 추적] 호출클래스=%s, 호출라인=%d, nErrorID=%d, nStatusNo=%d", "NormalMoHandler", 213, ERRORID_CP_MO_FAIL, ST_DB_INS_FAIL_GIPMOCALLINFO), workerThreadId)
                    smsQLib.InsqStat(
                        gstQItem, MESSAGE_MO, 0, gServerID, MODULEID_GIPEVENT_C, SERVICEID_GIPEVENT,
                        ERRORID_CP_MO_FAIL, ST_DB_INS_FAIL_GIPMOCALLINFO,
                        moQItemUtilService.getNInforNo(gstQItem), TID_NO_SAVE, LT_TRACE, 0
        ...
        // MOTRBILL=N: MOCALLINFO/Relay 저장 생략, dbInsertOk=true 유지
    ...
    override fun checkQItemVariables(qItem: QITEM, context: MoServiceContext): Boolean {
        ...
        if (msgId.isBlank()) {
            ...
            witcomLog.c_write(loggerName, Level.INFO, String.format("[InsqStat 추적] 호출클래스=%s, 호출라인=%d, nErrorID=%d, nStatusNo=%d", "NormalMoHandler", 292, ERRORID_CP_MO_FAIL, ST_GIPEVENT_MO_OK), workerThreadId)
            smsQLib.InsqStat(
        ...
        if (!hasTraceId) {
            ...
            witcomLog.c_write(loggerName, Level.INFO, String.format("[InsqStat 추적] 호출클래스=%s, 호출라인=%d, nErrorID=%d, nStatusNo=%d", "NormalMoHandler", 311, ERRORID_CP_MO_FAIL, ST_GIPEVENT_MO_OK), workerThreadId)
            smsQLib.InsqStat(
                qItem, MESSAGE_MO, 0, gServerID, MODULEID_GIPEVENT_C, SERVICEID_GIPEVENT,
                ERRORID_CP_MO_FAIL, ST_GIPEVENT_MO_OK, qItem.usSource, TID_NO_SAVE, LT_BOTH, 0
            )
    ...
    override fun recordMoSuccessInsqStat(qItem: QITEM, context: MoServiceContext) {
        ...
        val gServerID = context.gServerID ?: return
        val witcomLog = context.witcomLog ?: return
        witcomLog.c_write(context.loggerName, Level.INFO, String.format("[InsqStat 추적] 호출클래스=%s, 호출라인=%d, nErrorID=%d, nStatusNo=%d", "NormalMoHandler", 326, ERRORID_CP_MO_SUCCESS, ST_GIPEVENT_MO_OK), context.workerThreadId)
        smsQLib.InsqStat(
            qItem,
            MESSAGE_MO,
            0,
    ...
    override fun recordMoAckBilling(qItem: QITEM, context: MoServiceContext, billType: Char) {
        ...
        val gServerID = context.gServerID ?: return
        val witcomLog = context.witcomLog ?: return
        witcomLog.c_write(context.loggerName, Level.INFO, String.format("[InsqStat 추적] 호출클래스=%s, 호출라인=%d, nErrorID=%d, nStatusNo=%d", "NormalMoHandler", 346, com.infra.mo.skt_giphttp_mo.dto.jna.SmsDef.ERRORID_CP_MO_SUCCESS, ST_GIPEVENT_MOACK_BILL_OK), context.workerThreadId)
        smsQLib.InsqStat(
            qItem, MESSAGE_MO, 0, gServerID, MODULEID_GIPEVENT_C, SERVICEID_GIPEVENT,
